...
$('#party-time').on('click', function () {
  $('section').removeClass('boring');
  $('#row4 div').fadeOut(1500);
  $('.box').addClass('snazzy');
  $('#box3 span').addClass('fun');
  $('h2').slideUp('fast');
  $('#row3 div').addClass('selected');
  $('.add-para').append("<p class='removed'>Time to party!!!</p>");
  $('#box12').prepend("<p class='removed'>Wild!</p>");
})
